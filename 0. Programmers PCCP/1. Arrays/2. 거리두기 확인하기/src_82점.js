...
const findBadNeighbor = (arr, y, x) => {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (y + partitions_dy[i][0] < 0 || y + partitions_dy[i][0] >= MAP_SIZE)
            continue;
        if (x + partitions_dx[i][0] < 0 || x + partitions_dx[i][0] >= MAP_SIZE)
            continue;
        if (y + partitions_dy[i][1] < 0 || y + partitions_dy[i][1] >= MAP_SIZE)
            continue;
        if (x + partitions_dx[i][1] < 0 || x + partitions_dx[i][1] >= MAP_SIZE)
            continue;
        if (
            arr[y + partitions_dy[i][0]][x + partitions_dx[i][0]] === "X" &&
