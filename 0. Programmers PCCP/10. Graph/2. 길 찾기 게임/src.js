const findAndConnectLeft = (parent, child, traverse) => {
  if (parent.left) {
    console.log(`[left] [traverse] parent(${parent.n}): [${parent.x}, ${parent.y}], child(${child.n}): [${child.x}, ${child.y}]`);
    return traverse(parent.left, child);
  }
  console.log(`[left] parent(${parent.n}): [${parent.x}, ${parent.y}], child(${child.n}): [${child.x}, ${child.y}]`);
  parent.left = child;
}

const findAndConnectRight = (parent, child, traverse) => {
  if (parent.right) {
    console.log(`[right] [traverse] parent(${parent.n}): [${parent.x}, ${parent.y}], child(${child.n}): [${child.x}, ${child.y}]`);
    return traverse(parent.right, child);
  }
  console.log(`[right] parent(${parent.n}): [${parent.x}, ${parent.y}], child(${child.n}): [${child.x}, ${child.y}]`);
  parent.right = child;
}
...
const solution = (nodeinfo) => {
  const [rootNode, ...children] = nodeinfo.map(([x, y], i) => ({
    n: i + 1,
    y,
    x,
    left: null,
