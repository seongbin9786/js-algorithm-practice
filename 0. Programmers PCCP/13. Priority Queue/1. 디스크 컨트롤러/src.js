...
const findEarliestAndShortestTask = (jobs, currentTime, completedJobs) => {
    ...
    for (const [id, requestedAt, jobExecutionTime] of jobs) {
      ...
      // 같이 도착했으나 더 긴 작업
      if (earliestReqAt == requestedAt && earliestJobExeTime <= jobExecutionTime) {
        continue;
      }
  ...
  const findShortestTask = (jobs, currentTime, completedJobs) => jobs
    .filter(([id, reqAt]) => currentTime >= reqAt && !completedJobs.has(id))
    .sort(([, aReqAt, aExeTime], [, bReqAt, bExeTime]) => aExeTime - bExeTime || aReqAt - bReqAt);
  ...
  */
  ...
  console.log(solution([[7, 8], [3, 5], [9, 6]]));