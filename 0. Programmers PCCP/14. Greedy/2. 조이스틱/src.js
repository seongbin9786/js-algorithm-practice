const diffLoc = (loc1, loc2, totalLength) => {
    ...
    const reverseDir =
        loc1 > loc2 ? totalLength - loc1 + loc2 : totalLength - loc2 + loc1;
...
const diffAlpha = (alpha1, alpha2) => {
    ...
    // console.log("diffAlpha:", alpha1, alpha2);
    // 1. 정방향
    const normalDir = alpha1 >= alpha2 ? alpha1 - alpha2 : alpha2 - alpha1;

    // 2. 역방향 (Z=26 쪽으로 가거나, A=1으로 갈 수 있음)
    // length와 가까운 쪽을 length에서 빼줘야 함.
    // 0-indexed라서 +1 해줘야 됨
    const reverseDir =
        alpha1 >= alpha2 ? 25 - alpha1 + alpha2 + 1 : 25 - alpha2 + alpha1 + 1;
...
const solution = (name) => {
    ...
    const alphabets = [...name].map((alpha, i) => ({
        ...
        loc: i, // 0-indexed
        distance:
            diffAlpha(alpha, INIT_ALPHA) + diffLoc(i, INIT_LOC, name.length),
        visited: false,
    }));
