...
var threeSum = function (nums) {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    const uniquePairs = possibleDuplicatePairs.filter((candidatePair) => !candidatePair.includes(i));
    // O(n^2)
    uniquePairs.forEach((pairs) => {
      ...
      const [x, y, z] = triple;
      // check duplicates
      const isDuplicated = uniqueIndicesTriples.find(([a, b, c]) => a === x && b === y && c === z);
      if (isDuplicated) {
        return;
      }
  ...
  // 실제 값으로 매핑 O(n)
  const duplicateValuesTriples = uniqueIndicesTriples.map(([i, j, k]) => ([nums[i], nums[j], nums[k]]));
